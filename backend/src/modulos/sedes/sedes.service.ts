...
export class SedesService {
  ...
  async crear(tenantId: string, dto: CrearSedeDto) {
    ...
    const sedesActuales = await this.db.location.count({ where: { tenantId, deletedAt: null } });

    if (sedesActuales >= (tenant.maxLocations ?? 3)) {
      throw new BadRequestException(`Has alcanzado el límite de ${tenant.maxLocations} sedes de tu plan`);
    }

    return this.db.location.create({
      data: {
        tenantId, name: dto.nombre, code: dto.codigo, address: dto.direccion,
        city: dto.ciudad, stateProvince: dto.estadoProvincia, countryCode: dto.codigoPais,
        phone: dto.telefono, email: dto.email, taxName: dto.nombreImpuesto,
        taxRate: dto.tasaImpuesto, sortOrder: dto.ordenamiento,
      },
    });
  }
  ...
  async obtenerTodas(tenantId: string, paginacion: PaginacionDto) {
    ...
    const [datos, total] = await Promise.all([
      this.db.location.findMany({ where, skip: paginacion.skip, take: paginacion.take, orderBy: { sortOrder: 'asc' } }),
  ...
  async obtenerPorId(tenantId: string, id: string) {
    const sede = await this.db.location.findFirst({ where: { id, tenantId, deletedAt: null } });
  ...
  async actualizar(tenantId: string, id: string, dto: ActualizarSedeDto) {
    ...
    return this.db.location.update({
      where: { id },
      data: {
        name: dto.nombre, code: dto.codigo, address: dto.direccion, city: dto.ciudad,
        phone: dto.telefono, email: dto.email, taxName: dto.nombreImpuesto,
        taxRate: dto.tasaImpuesto, sortOrder: dto.ordenamiento,
      },
    });
  ...
  async eliminar(tenantId: string, id: string) {
    await this.obtenerPorId(tenantId, id);
    return this.db.location.update({ where: { id }, data: { deletedAt: new Date(), isActive: false } });
  }
